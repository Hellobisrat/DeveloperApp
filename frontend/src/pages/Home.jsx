      ...
      <div className="col-span-2 border border-l border-purple-100 rounded-lg shadow-lg m-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 m-4 md:m-6">
          {developers?.map((dev) => (
            <div
              key={dev._id}
              className="relative mt-12 py:3 px-1 md:py-6 md:px-1 w-full rounded-lg border-2 border-purple-200 shadow-md flex items-center gap-4 justify-evenly"
            >
              <div className="flex flex-col gap-2">
                ...
                <p className="text-purple-600 text-xs">
                  <span className="text-gray-900 text-sm">LName: </span>
                  {dev.lastName}
                </p>
              </div>
              ...
              <div>
                <div className="lg:flex flex-col gap-2 hidden">
                  ...
                  <p className="text-purple-600 text-sm">
                    <span className="text-gray-900 text-sm">Role:</span>{" "}
                    {dev.role}
                  </p>

                  <button
                    className="absolute top-0 right-0 p-1 flex flex-col items-center justify-center gap-2"
                    onClick={() =>
                      setEditId(editId === dev._id ? null : dev._id)
                    }
                  >
                  ...
                  {editId === dev._id && (
                    <div className="absolute top-0 right-0 w-40 z-50 rounded-md p-4 border border-purple-100 backdrop-blur-lg bg-white">
      ...
      <div className="hidden lg:block md:col-span-1 border border-l border-purple-100 rounded-lg shadow-lg bg-white m-3">
          ...
          <div className="border border-purple-100 shadow-md p-6">
            ...
            {LeadDevelopers.map((dev) => (
              <div key={dev._id} className="flex items-center justify-start gap-4">
                ...
                <p className="text-lg text-purple-300 w-32">{dev.name}</p>
              </div>
            ))}
          </div>
        </div>

        <hr className="border-t-2 border-purple-200 w-3/4 my-3 mt-10 mx-auto" />
        ...
        <div className="flex flex-col items-center mt-12 space-y-4">
          ...
          <div className="border border-purple-100 shadow-md rounded-lg p-6 mx-12 flex flex-col space-y-3 max-w-md">
            ...
            <p className="text-sm text-purple-600">Total Developer: {Stat.Total}</p>
          </div>
        </div>
      </div>
    </div>
